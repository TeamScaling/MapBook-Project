...
public class QuerySendChecker implements RestorationChecker{
     ...
     *
     * @param apiConnection 복원 가능성을 확인할 API 서버와의 연결 구성 요소를 갖는 {@link ApiConnection} 인스턴스
     * @return API 서버가 다시 접근 가능한 상태라면 true, 그렇지 않은 경우 false를 반환합니다.
     */
    public boolean isRestoration(ApiConnection apiConnection) {

        log.info("check restoration of Api [{}] at [{}]", apiConnection,LocalDateTime.now());

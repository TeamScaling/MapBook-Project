package com.scaling.libraryservice.dataPipe.updater.service;
...
/**
 * Book 데이터 셋을 최신화 하는 역할을 합니다. 최신화가 필요한 데이터 목록을 조회 한 뒤, 외부 API의 도움을 받아 데이터를 갱신 합니다.
 */
@Service
...
public class BookUpdateService {
    ...
    public void UpdateBookFromApi(int limit, int nThreads) {
        ...
        List<KakaoBookConn> kakaoBookConns = bookList.stream().map(KakaoBookConn::new).toList();
    ...
    /**
     * DB에서 가져올 최신화가 필요한 도서 목록의 제한 개수를 인자로 받아, 해당 도서 목록을 가져옵니다.
     *
    ...
    /**
     * 외부 API로부터 도서 상세 정보 데이터를 받아와, DB의 도서 데이터를 최신화합니다. 외부 API에서 해당 도서 정보를 제공하지 않는 도서는 DB에서 삭제
     * 처리됩니다.
     */
    ...
    private Map<String, UpdateBook> generateBookMap(@NonNull List<UpdateBook> bookList) {
        return bookList.stream()
            .collect(Collectors.toMap(UpdateBook::getIsbn, Function.identity()));
    }


    private void updateBookEntity(@NonNull List<BookApiDto> bookApiDtoList,
        Map<String, UpdateBook> map, long lastId) {
